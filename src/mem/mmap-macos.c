...
static inline uintptr_t guest_round_page(uintptr_t x)
{
...
__vm_allocate(vm_map_t task, vm_address_t *addr, vm_size_t size, int flags)
{
	if (flags & VM_FLAGS_ANYWHERE) {
		*addr = guest_round_page(map_base + (*addr & entropy_mask));
		flags &= ~VM_FLAGS_ANYWHERE;
	}
	int rv = vm_allocate(task, addr, size, flags);
	if (!rv) {
		map_base = guest_round_page(*addr + size);
	}
	return rv;
}
